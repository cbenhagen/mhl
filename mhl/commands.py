...
from .hasher import create_filehash, DirectoryContentHashContext, DirectoryStructureHashContext
...
def create_for_folder_subcommand(root_path, verbose, hash_format, no_directory_hashes, ignore_list=None, ignore_spec_file=None):
    ...
    dir_content_hash_context = DirectoryContentHashContext(hash_format)
    # store the structure directory hashes of sub folders so we can use it when calculating the hash of the parent folder
    dir_content_hash_mappings = {}
    # store the content directory hashes of sub folders so we can use it when needing a child history root content hash
    ...
    for folder_path, children in post_order_lexicographic(root_path, session.ignore_spec.get_path_spec()):
        ...
        dir_structure_hash_context = DirectoryStructureHashContext(hash_format)
        if no_directory_hashes:
            dir_structure_hash_context = None
        for item_name, is_dir in children:
            ...
            if is_dir:
                if not no_directory_hashes:
                    dir_structure_hash_context.append_subfolder_and_hash(item_name, dir_structure_hash_mappings.pop(file_path))
                    # add root content hash of nested history
                    history_relative_folder_path = existing_history.get_relative_file_path(folder_path)
                    child_history = existing_history.child_history_of_history(relative_path, history_relative_folder_path)
                    if child_history != None:
                        dir_content_hash_context.append_content_hash(relative_path, dir_content_hash_mappings.pop(file_path))
            else:
                # works for paths in existing_history, but also its child histories
                ...
                if not no_directory_hashes:
                    ...
                    dir_structure_hash_context.append_filename(item_name)
        dir_content_hash = None
        dir_structure_hash = None
        if not no_directory_hashes:
            relative_path = existing_history.get_relative_file_path(folder_path)
            dir_content_hash = dir_content_hash_context.final_content_hash_for_directory_prefix(relative_path)
            dir_structure_hash = dir_structure_hash_context.final_structure_hash()
...
@click.option('--directory_hash', '-dh', default=False, is_flag=True,
              help="Create new directory hashes and verify them against directory hashes stored in the history.")
@click.option('--directory_content_only', '-dco', default=False, is_flag=True,
              help="Option for the --directory_hash / --dh sub-command: Compare content directory hashes only.")
@click.option('--directory_structure_only', '-dso', default=False, is_flag=True,
              help="Option for the --directory_hash / --dh sub-command: Compare content directory hashes only.")
@click.option('--hash_format', '-h', type=click.Choice(ascmhl_supported_hashformats),
              multiple=False,
              help="Algorithm")

def verify(root_path, verbose, directory_hash, directory_content_only, directory_structure_only, hash_format, ignore_list, ignore_spec_file):
    """
    ...
    if directory_hash is True:
        verify_directory_hash_subcommand(root_path, verbose, hash_format, directory_content_only, directory_structure_only, ignore_list, ignore_spec_file)
        return
...
def verify_directory_hash_subcommand(root_path, verbose, hash_format,
                                     directory_content_only, directory_structure_only,
    ...
    for folder_path, children in post_order_lexicographic(root_path, ignore_spec.get_path_spec()):
        ...
        # compare regular directory hash
        ...
        for directory_hash_entry in directory_hash_entries:
            ...
            num_current_successful_verifications = _compare_and_log_directory_hashes(relative_path,
                                                                                     directory_hash_entry, dir_content_hash,
                                                                                     dir_structure_hash,
                                                                                     directory_content_only,
                                                                                     directory_structure_only,
                                                                                     )
...
def _compare_and_log_directory_hashes(relative_path, directory_hash_entry,
    ...
    if directory_content_only == False and directory_structure_only == False:
    ...
    elif directory_content_only == True:
        if directory_hash_entry.hash_string == calculated_content_hash_string:
            if relative_path == ".":
                logger.verbose(f'  content verification of root folder   OK '
                               f'(generation {directory_hash_entry.temp_generation_number:04d})')
            else:
                logger.verbose(f'  content verification of folder        {relative_path}{root_string} OK '
                               f'(generation {directory_hash_entry.temp_generation_number:04d})')
    ...
    elif directory_structure_only == True:
        if directory_hash_entry.structure_hash_string == calculated_structure_hash_string:
            if relative_path == ".":
                logger.verbose(f'  structure verification of root folder   OK '
                               f'(generation {directory_hash_entry.temp_generation_number:04d})')
            else:
                logger.verbose(f'  structure verification of folder        {relative_path}{root_string} OK '
                               f'(generation {directory_hash_entry.temp_generation_number:04d})')
